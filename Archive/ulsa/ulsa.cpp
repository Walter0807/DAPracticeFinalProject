      ...
      for (int j = 0; j < numvarneighbors[c]; j++)
      {
        ...
        for (int k = 0; k < domain; k++)
        {
          if (whichconstraint[c][i][varneighbors[c][j]] & (1ULL << k))
            varneighborvector[c][i][j][k / WORDDOMAINMEMBERS] |= (1LL << ((k % WORDDOMAINMEMBERS) * (BITSPERDOMAINMEMBER)));
        }
      }
    }
  }
...
void setrandvar()
{
  ...
  int unsetvar = allunset[random() % numallunset];
  ...
  setvar(unsetvar, bestsval[random() % numbests]);
}
  ...
  if (successgap >= 2)
  {
    for (int i = 0; i < numtwo; i++)
    {
      for (int j = i + 1; j < numtwo; j++)
      {
        ...
        int intra = 0;
        if (whichconstraint[two[i]][varval[two[i]]][two[j]] & (0x1LL << (varval[two[j]])))
          intra = 1;
        ...
        if (successgap >= 3)
        {
          for (int k = j + 1; k < numtwo; k++)
          {
            ...
            int intra3 = intra;
            if (whichconstraint[two[i]][varval[two[i]]][two[k]] & (0x1LL << (varval[two[k]])))
              intra3++;
            if (whichconstraint[two[j]][varval[two[j]]][two[k]] & (0x1LL << (varval[two[k]])))
              intra3++;
...
long long int sls(long long maxupdates, int successgap, int successgapcheckviolations, int printviolations)
{
  ...
  while (numupdates <= maxupdates)
  {
    ...
    int vindex = random() % numviolations;
...
int main(int argc, char *argv[])
{
  ...
  srandom(seed);
  sscanf(argv[3], "%d", &vars);
  sscanf(argv[4], "%d", &domain);
  ...
  long long int runlength;
  sscanf(argv[5], "%lld", &runlength);
  int numruns;
  sscanf(argv[6], "%d", &numruns);
  int successgap, successgapcheckviolations, printviolations;
  sscanf(argv[7], "%d", &successgap);
  sscanf(argv[8], "%d", &successgapcheckviolations);
  sscanf(argv[9], "%d", &printviolations);
